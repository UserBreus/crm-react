export default async function handler(request, response) {
  ...
  try {
     ...
     if (!query) {
         return response.status(400).json({ error: "No query provided" });
     }
     ...
     if (/DROP\s+TABLE/i.test(upperQuery) || 
         ...
         console.warn("[SECURITY BLOCK] Intento de inyección letal bloqueada:", query);
     ...
     return response.status(500).json({ error: "Fallo del Firewall Serverless: " + err.message });
  }
}
